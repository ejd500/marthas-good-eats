...
var getMenuItemById = function(menu_id) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    pgDAL.query(sql, [menu_id], (err, result) => {
      if (err) {
        if(DEBUG) console.log(err);
        reject(err);
...
var addMenuItem = function(name, description, price, category, image_url) {
  ...
  return new Promise(function(resolve, reject) {
    const sql = `INSERT INTO public."Menu_Items"(name, description, price, category, image_url) \
        VALUES ($1, $2, $3, $4, $5);`
    pgDAL.query(sql, [name, description, price, category, image_url], (err, result) => {
      if (err) {
          if(DEBUG) console.log(err);
          reject(err);
        } else {
...
var patchMenuItem = function(menu_id, name, description, price, category, image_url) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    pgDAL.query(sql, [menu_id, name, description, price, category, image_url], (err, result) => {
      if (err) {
          reject(err);
        } else {
          resolve(result.rows);
